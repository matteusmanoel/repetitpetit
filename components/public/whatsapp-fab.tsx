...
const SUPPORT_MESSAGE = "Oi, preciso de ajuda!";
const REVEAL_DELAY_MS = 1500;

type WhatsAppFabProps = {
  /** Número com DDI + DDD, apenas dígitos (ex.: 554599999999). Vem de env.NEXT_PUBLIC_STORE_WHATSAPP. */
  whatsappNumber?: string;
};

/**
 * Botão flutuante de contato via WhatsApp. Link `wa.me` puro — sem agente
 * conversacional, sem copy floral (ver docs/05-ux-direction.md e AGENTS.md).
 */
export function WhatsAppFab({ whatsappNumber }: WhatsAppFabProps) {
  ...
  if (!whatsappNumber || isAdminRoute || !isRevealed) {
    return null;
  }

  const href = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(SUPPORT_MESSAGE)}`;
